...
if page == "Accueil":
    ...
    metric = st.selectbox(
    label="Sélectionner la métrique",
    ...
    if st.button("ANALYSER LE CAS", type="primary", use_container_width=True):
        if not valid:
            ...
            if model is None:
                ...
                # Gestion robuste de la confiance : certains SVC n'ont pas `predict_proba`
                if hasattr(model, "predict_proba"):
                    proba = model.predict_proba(X)[0]
                else:
                    # fallback : utiliser decision_function puis softmax/logistic pour obtenir des valeurs comparables
                    try:
                        scores = model.decision_function(X)
                        # binaire -> decision_function retourne un scalaire
                        if np.ndim(scores) == 1:
                            # Convertir la marge en probabilité approchée via la sigmoïde
                            prob_pos = 1.0 / (1.0 + np.exp(-scores))[0]
                            proba = np.array([1.0 - prob_pos, prob_pos])
                        else:
                            # multiclasses -> softmax
                            exps = np.exp(scores - np.max(scores, axis=1, keepdims=True))
                            proba = (exps / np.sum(exps, axis=1, keepdims=True))[0]
                    except Exception:
                        # Si tout échoue, on met une confiance totale sur la classe prédite
                        proba = np.array([1.0, 0.0]) if pred == 0 else np.array([0.0, 1.0])

                classe = DATASET_INFO[dataset]['target_names'][pred]
                confiance = float(proba[pred]) * 100.0

                css_class = 'benin' if pred == 0 else 'malin'
                st.markdown(f"<div class='prediction-box {css_class}'>RÉSULTAT : {classe.upper()}</div>",
                            unsafe_allow_html=True)
